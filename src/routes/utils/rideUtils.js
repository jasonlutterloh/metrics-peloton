...
export const filterRidesByDate = (rideData, dateFilters) => {
  if (
    ...
    return rideData.filter((ride) => {
      return (
        isDateSameOrAfterGivenDate(ride.date, dateFilters.startDate) &&
        isDateSameOrBeforeGivenDate(ride.date, dateFilters.endDate)
      );
    });
  }
  return rideData;
...
export const getOrganizedRidesSortedByOutput = (rideData) => {
  ...
  for (const [i, duration] of durations.entries()) {
    newOrganizedRides[duration] = sortArrayByAttributeInObject(rideData[duration], "output");
  }
