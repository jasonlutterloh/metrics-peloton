 ...
 */
export const rideTitleFilters = writable([
  "Intervals & Arms",
  "Cool Down",
  "Warm Up",
  "Recovery",
 ...
 */
export const mappedCSVData = derived(
 ...
 */
export const rideTypes = derived(mappedCSVData, ($mappedCSVData) =>
  getUniqueRideTypes($mappedCSVData)
);
 ...
 */
export const totalDistance = derived(filteredData, ($filteredData) =>
  getTotalByAttribute($filteredData, "distance")
);

/**
 * Store of total calories burned in all filtered rides
 */
export const totalCalories = derived(filteredData, ($filteredData) =>
  getTotalByAttribute($filteredData, "calories")
);

/**
 * Store of total minutes spent of all filtered rides
 */
export const totalMinutes = derived(filteredData, ($filteredData) =>
  getTotalByAttribute($filteredData, "duration")
);

/**
 * Store of average outputs of all rides
 */
export const averageOutputs = derived(filteredData, ($filteredData) =>
  getAverageOutputs($filteredData)
);
...
export const ftpAverageOutputs = derived(ftpTestRides, ($ftpTestRides) =>
  getAverageOutputs($ftpTestRides, energy.WATTS)
);

/**
 * Store of the average cadences for all filtered rides
 */
export const averageCadence = derived(filteredData, ($filteredData) =>
  getAverageCadence($filteredData)
);

/**
 * Store of the average resistances for all filtered rides
 */
export const averageResistance = derived(filteredData, ($filteredData) =>
  getAverageResistance($filteredData)
);

/**
 * Store of the count of classes taken by each instructor
 */
export const classesTakenPerInstructor = derived(filteredData, ($filteredData) =>
  getClassesTakenByInstructor($filteredData)
);

/**
 * Store of the average outputs organized by ride type
 */
export const averageOutputByRideType = derived(filteredData, ($filteredData) =>
  getAverageOutputByRideType($filteredData)
);

/**
 * Store of the average outputs by instructor
 */
export const averageOutputByInstructor = derived(filteredData, ($filteredData) =>
  getAverageOutputByInstructor($filteredData)
);
